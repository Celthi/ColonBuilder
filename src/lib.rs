    ...
    match fields {
        Fields::Named(fs) => {
            fs.named.iter().for_each(|f| {
                ...
                let abbr = get_abbr(f);
                let field_name = if abbr.is_some() {
                    abbr.unwrap()
                } else {
                    format!("{}", ident)
                };
                let regexp = get_regexp(field_name);
    ...
    if ty == &vec_string_type {
        quote! {
            ...
            pub fn #method_name(s: &str) -> Vec<String> {
                ...
                if let Some(m) = pat.captures(s) {
                    let v = m.name("v").map(|s| s.as_str().trim().to_owned()).unwrap_or("".to_string());
                    let tc: Vec<String> = v
                    .split(',')
                    .into_iter()
        ...
        quote! {
            ...
            pub fn #method_name(s: &str) -> String {
                let pat = reg!( #regexp);
                if let Some(m) = pat.captures(s) {
                    m.name("v").map(|s| s.as_str().trim().to_owned()).unwrap_or("".to_string())
                } else {
                    "".to_string()
                }                        }
        }
